<template>
  <div class="installment">
    <applyHeader ref="applyheader" styleName="5" />
    <div class="sohpp-tip">
      <div>
...
export default {
  ...
  methods: {
    ...
    qrcode (item) {
      ...
      canvas.innerHTML = ''
      var link = location.origin + '/stagemall_page/home?siteId=' + item.id
    ...
    sbmitlist () {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          for (let index = 0; index < data.resultStageList.length; index++) {
            const item = data.resultStageList[index]
          ...
          this.siteList = data.resultStageList
        }
      })
    },
    packlist () {
    ...
    iscreatedSite () {
      if (this.orgInfo === null || (this.orgInfo.overdueFlag === '1' && this.orgInfo.vipType === 3 && this.siteList.length < 10)) {
        return false
      }
      return true
    },
    addSite () {
      ...
      if (orgType === 2) {
        this.$router.push({ path: '/basicInfo', query: { type: 2 } })
      } else {
        this.$router.push({ path: '/choiceModel', query: { type: 2 } })
      }
    }
  },
  components: {
