  ...
  <div class="wapper">
    ...
    <div class="pannelT">
      ...
      <dl>
        ...
        <dd>
          ...
          <span>（余额支付：{{(form.payAmountYuan - form.cashAmountYuan).toFixed(2) || '0.00'}}</span>
          <span v-if="form.payMode!=null && form.payMode !=form.payWay">
            +
            <span v-html="getpayMode(form.payWay)"></span>
            ：{{form.cashAmountYuan |currency}}
          </span>）
        </dd>
        <dd v-show="payNo!=''">
          支付流水号：
          <el-tooltip width="230px" placement="top">
            <div slot="content">仅驿宝通财务去支付平台退款用</div>
        ...
        <dd>
          ...
          <span>（商品金额：{{form.totalSalePrice==null?'0:00':form.totalSalePrice}}）</span>+
          <span>（运费：{{form.freight |currency}}）</span>
          <span v-if="form.benefitAmount">- （优惠金额：{{form.benefitAmount |currency}}）</span>
        ...
        <dd>
          商城支付金额：
          <span v-html="adddisPrice()"></span>元&nbsp;=
          <span>（商品金额：{{form.payDistribYuan==null?'0:00':form.payDistribYuan}}）</span>+
          <span>（运费：{{form.freight |currency}}）</span>
          <span v-if="form.payMode!=null && form.cashAmountYuan">
            -（
            <span v-html="getpayMode(form.payWay)"></span>
            ：{{form.cashAmountYuan |currency}}）
          </span>
        ...
        <dd>
          机构支付金额：
          <span v-html="addPrice()"></span>元&nbsp;=
          <span
            v-if="form.paySupplyYuan!='' && source!=2"
          >（商品金额：{{form.paySupplyYuan |currency}}）+</span>
          （运费：{{form.supplyFreight |currency}}）+（扣点：{{serviceFee}}）
        </dd>
      </dl>
    ...
    <div class="pannelM">
      ...
      >
        ...
        <el-table-column prop="benefitAmount" label="优惠金额"></el-table-column>
        <el-table-column prop="itemName" label="订单类型">
          <template slot-scope="scope">
            <span style="display:none">{{scope.row.itemName}}</span>
            <div v-html="valiOrderType(form.orderType)"></div>
          </template>
        </el-table-column>
        <el-table-column prop="supplyName" label="供应商">
          <template slot-scope="scope">{{form.orderType==7?'猫眼电影':scope.row.supplyName}}</template>
        </el-table-column>
        <el-table-column prop="channelChilerenNo" label="渠道子单号">
          <template slot-scope="scope">{{scope.row.channelChilerenNo || '— — '}}</template>
        </el-table-column>
        ...
        <el-table-column prop="trackState" label="操作" width="200" v-if="source!=9">
          <template slot-scope="scope">
            <div v-if="roleType==2 && scope.row.operStatus==3">
              <el-popover
                ...
                v-if="scope.row.operStatus!=null"
              >
                <div class="vcontent">
                  <div class="vl">
                    <ul>
                      <li v-for="(item,index) in auditLists" :key="index">
                        ...
                        <h2>
                          {{item.operStatusName}}
                          <span>{{item.endTime==null?item.startTime:item.endTime}}</span>
                        </h2>
                        <p v-html="getcontent(item.operStatus)"></p>
                        <p
                          v-if="item.operStatus==3"
                        >物流信息：{{setExpress.expressCompanyName}} {{setExpress.expressNumber}}</p>
                        <!--售后物流 -->
                        <el-popover
                          placement="right"
                          width="400"
                        ...
                        <el-button
                          type="primary"
                          size="mini"
                          @click="importdanhao(item,1)"
                          v-if="item.operStatus==2 && finaaloperStatus==2"
                        >录入单号</el-button>
                        <el-button
                          type="primary"
                          size="mini"
                          @click="importdanhao(item,0)"
                          v-if="item.operStatus==3 && finaaloperStatus!=5 "
                        >修改</el-button>
                        <div v-if="item.operStatus==1 && item.approvalJson==null">
                          ...
                          <br />
                          <span
                            style="font-size:12px;padding-top:10px;color:red;display: block;"
                          >务必与供应商确认是否已发货，否则将会导致退款错误！</span>
                        </div>
                        <!--  item.operStatus==1 && item.approvalJson==null || item.operStatus==4 && item.approvalJson!='' || item.operStatus==3 -->
                        <div
                          class="d1"
                          v-if="finaaloperStatus==3 && item.operStatus==3 || finaaloperStatus==1 && item.operStatus==1 || finaaloperStatus==4 && item.operStatus==4 "
                        >
                          <el-button
                            type="success"
                            size="mini"
                            @click="shouhoustate(item,1)"
                            :disabled="disabled"
                          >通过</el-button>
                          <el-button
                            type="danger"
                            size="mini"
                            @click="shouhoustate(item,2)"
                            :disabled="disabled"
                          >不通过</el-button>
                          <el-button
                            type="info"
                            size="mini"
                            @click="shouhoustate(item,4)"
                            :disabled="disabled"
                          >撤销</el-button>
                        </div>
                      </li>
                    ...
                    <div class="d1">
                      退款原因：{{saasOrderAfterServ.reason}}
                      <br />
                      <div
                        v-if="finaaloperStatus>3 && finaaloperStatus!=14 && finaaloperStatus!=15"
                      >
                        <span v-if="source!=2">
                          退给会员：{{saasOrderAfterServ.refundOrgAmount/100}}
                          <br />
                        </span>
                        退给商城：{{saasOrderAfterServ.refundSiteAmount/100}}
                        <br />
                        退给用户：{{saasOrderAfterServ.totalprice}} （现金:{{saasOrderAfterServ.refundCashAmount/100}}；余额:{{saasOrderAfterServ.refundPointAmount/100}}；运费:{{saasOrderAfterServ.refundFeight/100}}）
                        <br />
                      </div>
                      联系电话：{{saasOrderAfterServ.mobile}}
                      <br />
                      问题描述：{{saasOrderAfterServ.description}}
                      <br />图片：
                      <el-tooltip
                        placement="top"
                        effect="light"
                        v-for="(itemd,indexb) in saasOrderAfterServ.fileAttachment"
                        :key="indexb"
                      >
                        <div slot="content">
            ...
            <div v-if="roleType==1 && form.orderType!=1">
              <el-button
                type="primary"
                size="mini"
                @click="applyCustomerService(scope.row)"
                v-if="form.operStatus==22 && scope.row.operStatus==null || form.operStatus==23 && scope.row.operStatus==null"
                :ref="scope.row.orderSubno"
              >申请售后</el-button>
                  ...
                  <div class="vl">
                    <ul>
                      <li v-for="(item,index) in auditLists" :key="index">
                        ...
                        <div
                          class="d1"
                          v-if="finaaloperStatus==3 && item.operStatus==3 || finaaloperStatus==1 && item.operStatus==1 ||userInfo=='0/1/' && createUserId==1 && finaaloperStatus==4 && item.operStatus==4"
                        >
                          <div>
                            <el-button
                              type="success"
                    ...
                    <div class="d1">
                      ...
                      >
                        ...
                        退给商城：{{saasOrderAfterServ.refundSiteAmount/100}}
                        <br />
                        退给用户：{{saasOrderAfterServ.totalprice}} （现金:{{saasOrderAfterServ.refundCashAmount/100}}；余额:{{saasOrderAfterServ.refundPointAmount/100}}；
                        <span>运费{{saasOrderAfterServ.refundFeight/100}}</span>）
                        <br />
                      </div>
                      联系电话：{{saasOrderAfterServ.mobile}}
                      <br />
        ...
        <div class="order-nav">
          ...
          >
            ...
            <div v-else style="text-align:center;margin-top:10px;">
              <img src="../img/noOrder.jpg" style="width:240px" />
              <br />请核实填写的物流单号是否准确或物流公司还未录入系统！可点击修改再次输入！
            </div>
          </div>
        </div>
    ...
    <el-dialog
      title="申请售后"
      :visible.sync="showapplyCustomerService"
      width="800px"
      :closeOnClickModal="false"
      class="applyCustomerStyle"
    >
      <el-form ref="form" :model="apply" class="formPannel">
        ...
        <el-form-item label="退货原因" label-width="80px">
          <el-select v-model="apply.rerson" class="w140" placeholder="全部">
        ...
        <el-form-item label="退货数量" label-width="80px">{{apply.num}}</el-form-item>
        <el-form-item label="问题描述" label-width="90px">
    ...
    <el-dialog
      title="录入单号"
      :visible.sync="showapplyDelivery"
      width="600px"
      :closeOnClickModal="false"
    >
...
import { accAdd, accSub } from '@/utils/index'
...
export default {
  ...
  methods: {
    ...
    shouhoustate (item, type) {
      ...
      if (type === 1) {
        this.$alert('确定后将开始售后，流程见右侧说明，请确认！', '提示', {
          ...
          this.disabled = true
          this.$http({
            url: this.$http.adornUrl('/ybt-backend/order/saasOrderAfterSupport/audit'),
            method: 'post',
            data: this.$http.adornData(
              {
                id: item.id,
      ...
      if (type === 2 || type === 4) {
        this.$confirm('确定后将不会退款，且无法再次申请售后，请确认!', '提示', {
          confirmButtonText: '确定'
          // cancelButtonText: '取消',
          // type: 'warning'
        }).then(() => {
    ...
    sureapply () {
      this.disabled = true
      this.$http({
        url: this.$http.adornUrl('/ybt-backend/order/saasOrderAfterServ/apply'),
        method: 'post',
        data: this.$http.adornData(
          {
            type: 1,
    ...
    adddisPrice () {
      if (this.form.payDistribYuan === undefined || this.form.freight === undefined) {
        return false
      }
      let a = accAdd(this.form.payDistribYuan == null ? '0' : this.form.payDistribYuan, this.form.freight)
      if (this.form.cashAmountYuan == null) {
        this.form.cashAmountYuan = 0
      }
      let b = accSub(a, this.form.cashAmountYuan)
      return b
    },
    addPrice () {
      ...
      if (Number(this.source) === 2) {
        this.form.paySupplyYuan = 0
      }
    ...
    getserviceFee () {
      this.$http({
        url: this.$http.adornUrl('/ybt-backend/order/order/queryOrderServFee'),
        method: 'post',
        data: this.$http.adornData(
          {
        ...
        if (data && data.data.code === 0) {
          var serviceFee = data.data.serviceFee || '0.00'
    ...
    delivery () {
      this.$http({
        ...
        if (data && data.data.code === 0) {
          ...
          res.order.freight = parseFloat(res.order.freight).toFixed(2)
          this.form = res.order
          ...
          if (this.source === 2 || this.source === 102) {
            ...
            if (res.track.msg && res.track.msg.length > 0 && res.track.msg !== '暂无物流信息' && res.track.msg !== '订单取消，无物流信息') {
