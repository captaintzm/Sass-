<template>
  <div class="siteWrapper">
    ...
    <el-form ref="form" label-width="85px" size="medium">
      ...
      <el-form-item label="流水号" class="formItem">
        <el-input
    ...
    >
      <el-table-column prop="amountYuan" label="发票总额" width="180"></el-table-column>
      <el-table-column prop="invoicesNo" label="申请流水号"></el-table-column>
      <el-table-column prop="createDate" label="申请时间"></el-table-column>
      <el-table-column prop="orgName" label="所属机构" v-if="orgType==2"></el-table-column>
      ...
      <el-table-column prop label="操作">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-if="orgType==2 && scope.row.operStatus==1"
          >不通过</el-button>
           <el-button  type="text" size="small" v-if="orgType==2 && scope.row.operStatus==2 && scope.row.logisticsStatus!=1" @click="writedetail(scope.row)">填写发票物流</el-button>
    ...
    <apiInvoicingdetail ref="apiInvoicingdetail" />
    <!-- 查看发票物流 -->
    <invoiceLook ref="invoiceLooks"/>
    <invoiceWrite ref="invoiceWrites" @click="getpacklist"/>
  </div>
</template>
...
import apiInvoicingdetail from '@/components/Finance/apiInvoicingdetail'
...
export default {
  ...
  methods: {
    ...
    importReport () {
      ...
      this.$http({
        url: this.$http.adornUrl(
          '/ybt-backend/apiInvoice/exportApiInvoicesApplyList'
        ),
        method: 'post',
    ...
    Applyticket () {
      this.$router.push({ path: '/apigoodsInvoicing' })
    },
    detail (item) {
      this.$refs.apiInvoicingdetail.savedo(item.id, 1)
      this.$refs.apiInvoicingdetail.dialogTableVisible = true
    },
    pass (item) {
      this.$confirm('是否确认通过该发票审核？', '审核提示', {
        ...
        this.$http({
          url: this.$http.adornUrl('/ybt-backend/apiInvoice/uptOperStatus'),
          method: 'post',
          data: this.$http.adornData({
            id: item.id,
            operStatus: 2,
    ...
    packlist () {
      ...
      this.$http({
        url: this.$http.adornUrl('/ybt-backend/apiInvoice/queryApiInvoicesApplyList'),
        method: 'post',
        data: this.$http.adornData({
          startDate: this.orderTime === '' || this.orderTime === null ? '' : this.orderTime[0],
          endDate: this.orderTime === '' || this.orderTime === null ? '' : this.orderTime[1],
