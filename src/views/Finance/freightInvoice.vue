...
export default {
  ...
  methods: {
    ...
    importReportkaipiao (item) {
      this.$http({
        url: this.$http.adornUrl(
          '/ybt-backend/invoices/invoicesinfo/exportInvoiceInfoFreight'
        ),
        method: 'post',
        ...
        if (res) {
          ...
          link.download = '机构运费发票明细' + item.invoicesNo + '.xls'
    ...
    detail (item) {
      this.$refs.InvoicingDetail.comefromFreight(1)
      this.$refs.InvoicingDetail.savedo(item.id, 3)
