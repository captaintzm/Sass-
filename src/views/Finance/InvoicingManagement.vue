<template>
  <div class="siteWrapper">
    ...
    >
      ...
      <el-table-column prop="invoicesNo" label="申请流水号" width="180"></el-table-column>
      ...
      <el-table-column prop label="操作">
        <template slot-scope="scope">
          ...
          >不通过</el-button>
          <!-- orgType==2 && -->
          <el-button @click="importReportkaipiao(scope.row)" type="text" size="small" v-if="orgType==2 && scope.row.operStatus==2">导出（开票）</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="packlist">
...
export default {
  ...
  methods: {
    ...
    importReportkaipiao (item) {
      this.$http({
        url: this.$http.adornUrl(
          '/ybt-backend/invoices/invoicesinfo/exportInvoiceInfo'
        ),
        method: 'post',
        ...
        if (res) {
          ...
          link.download = '机构商品发票明细' + item.invoicesNo + '.xls'
    ...
    Applyticket () {
      this.$router.push({ path: '/goodsInvoicing' })
    },
    detail (item) {
      this.$refs.InvoicingDetail.savedo(item.id, 1)
