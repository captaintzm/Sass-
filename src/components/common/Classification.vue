...
export default {
  ...
  methods: {
    ...
    getqueryLable () {
      this.$http({
        ...
        if (res && res.data.code === 0) {
          this.mallCateLables = res.data.mallCateLables
        } else {
    ...
    getThirdCateLevelList (levelId) {
      this.$http({
        url: this.$http.adornUrl('/ybt-backend/product/product/getThirdCateLevelListByLevelId'),
        method: 'post',
        data: this.$http.adornData({
          levelId: levelId
        })
        ...
        if (res && res.data.code === 0) {
          this.mallcateProduct = res.data.supplyCateProductList
        } else {
          this.$message.error(res.data.msg)
        }
      })
