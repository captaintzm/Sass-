<template>
  ...
  >
   <span slot="title">
      ...
      <el-tooltip class="item" effect="light" content="发票总额=商品发票金额+运费发票内容+服务费" placement="top">
        <i class="tip_icon"></i>
      </el-tooltip>
  </span>
    <div class="siteWrapper">
      ...
      <el-form ref="form" label-width="100px" size="medium">
          <el-form-item label="发票总额" class="formItem formItemA">{{form.amountYuan}}</el-form-item>
        <el-form-item label="商品发票金额" class="formItem formItemA">{{form.productAmountYuan || 0}}</el-form-item>
        <el-form-item label="运费发票金额" class="formItem formItemA">{{form.freightAmountYuan || 0}}</el-form-item>
        <el-form-item label="服务发票金额" class="formItem formItemA">{{form.feeAmountYuan || 0}}</el-form-item>
          ...
          <tbody v-for="(item,index) in tableData" :key="index">
            <tr v-for="(childrenitem,childrenindex) in item.orderDetails" :key="childrenindex">
              ...
              <td
                ...
                v-if="item.orderDetails.length>0&&childrenindex==0"
              >{{item.amountYuan}}</td>
              <td>{{childrenitem.feeAmountYuan || 0}}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default {
  ...
  methods: {
    ...
    savedo (id, flag) {
      this.$http({
        url: this.$http.adornUrl('/ybt-backend/apiInvoice/info'),
        method: 'post',
        data: this.$http.adornData({
          id: id
        }, true)
