  ...
  >
    <div class="siteWrapper">
      ...
      <el-form ref="form" label-width="100px" size="medium">
        ...
        <el-form-item label="发票总额" class="formItem formItemA">{{info.amountYuan}}</el-form-item>
        <el-form-item label="商品发票金额" class="formItem formItemA">{{info.productAmountYuan}}</el-form-item>
        <el-form-item label="运费发票金额" class="formItem formItemA">{{info.freightAmountYuan}}</el-form-item>
        <el-form-item label="服务发票金额" class="formItem formItemA">{{info.feeAmountYuan}}</el-form-item>
...
export default {
  ...
  methods: {
    ...
    save () {
      this.$http({
        url: this.$http.adornUrl('/ybt-backend/apiInvoice/save'),
        method: 'post',
    ...
    importReport () {
      ...
      this.$http({
        url: this.$http.adornUrl(
          '/ybt-backend/apiInvoice/exportApiInvoicesOrderList'
        ),
        method: 'post',
    ...
    savedo () {
      this.$http({
        url: this.$http.adornUrl('/ybt-backend/apiInvoice/savedo'),
        method: 'post',
        data: this.$http.adornData()
      }).then(res => {
        if (res && res.data.code === 0) {
